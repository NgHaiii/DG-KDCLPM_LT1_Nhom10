...
@section('page-title', 'Bảng lương bác sĩ')
...
<div class="sheet">
    <div class="sheet-head">
        <div>
            <div class="title">BẢNG LƯƠNG BÁC SĨ</div>
            <div class="meta">
    ...
    <div class="actions-row">
        @if($payroll->status === 'draft')
            <form method="POST" action="{{ route('admin.payroll.approve', $payroll) }}">
                @csrf
                @method('PATCH')
                <button class="btn btn-primary" onclick="return confirm('Duyệt phiếu lương này?')">
        ...
        @if(in_array($payroll->status, ['draft', 'approved'], true))
            <form method="POST" action="{{ route('admin.payroll.mark-paid', $payroll) }}">
                @csrf
                @method('PATCH')
                <button class="btn btn-success" onclick="return confirm('Đánh dấu đã thanh toán?')">
